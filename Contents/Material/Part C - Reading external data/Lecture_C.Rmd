...
* Packages
* Reading CSV, STATA and Excel-files
...
- Until now we have worked maily with the functionality in `base`:
...
## Installation
 ...
 - Automatically installed dependencies if needed
...
## Use it
 Now we can call the functions and datasets in the package just like other objects we
...
- Good places to start:
  ...
  - If you are into official statistics: [github.com/SNStatComp/awesome-official-statistics-software](https://github.com/SNStatComp/awesome-official-statistics-software) and [https://cran.r-project.org/web/views/OfficialStatistics.html](https://cran.r-project.org/web/views/OfficialStatistics.html)
...
## Example
- I have created a small textfile called "example.txt" and put it in the path "C:/Users/Jolien/example.txt"
...
boys_csv[1:5,]
```
- CSV-files do not have the datatype `factor` and it is cast as `character`
- Always inspect your data after importing!

# Databases
## Databases
- Tabular data can be stored in relational databases like:
  * Oracle (`ROracle`)
...
- All functions start with `db` and then a verb e.g. `dbConnect`
...
cat("<SQLiteConnection>
  Path: C:\\Users\\Jolien\\boys_database.sqlite
  Extensions: TRUE")
```
...
dbWriteTable(con,
             ...
             value = boys)
```
We can now see that a table has been created:
```{r}
dbListTables(con)
```
...
## Other databases
- `SQlite` is often not a good candidate for a database. When working with official statistics you want a more feature complete database running on a dedicated server.
...
## Microsoft Access
[https://stackoverflow.com/questions/45816062/connect-ms-access-database-to-r](https://stackoverflow.com/questions/45816062/connect-ms-access-database-to-r)
