...
`vec1` and `mat1` contain numbers and `vec2` and `mat2` contain characters. Note that you can also use the `typeof` function to obtain the type of an object.
...
Again, make a dataframe called `dat3b` where `vec1` and `vec2` are both columns. Name the columns `V1` and `V2`, respectively. Use function `as.data.frame()` on the matrix obtained from Question 2.4.
...
Check if the first column in the data frames from Question 3.1 and Question 3.2 are indeed numeric. If not, determine what they are.
...
is.numeric(dat3b[, 1])
```
The first column in matrix `dat3b` obtained from Question 3.2 is indeed not numeric but character. 
```{r}
is.character(dat3b[, 1])
```

Note that in earlier versions of R, the function `as.data.frame` that we used to create `dat3b` automatically converts all character type data to factors. In that case the first column in the dataframe `dat3b` will be neither `numeric` nor `character` but instead will be `factor`.

`r unhide()`

### Exercise 3.4 
Select 1) the third row, 2) the second column and 3) the intersection of these two in the dataframe `dat3` that you have created in Question 3.1.
...
Note that `bmi` contains 21 missing values, e.g. by looking at the summary information. Therefore we need to use `na.rm = T` to calculate the standard deviation on the observed cases only.
`r unhide()`
