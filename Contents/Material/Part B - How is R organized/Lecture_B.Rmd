...
In this lecture we will look the following subjects:  
  ...
  * Basic datatypes
  * Vectors, matrices, data frames, tibbles and lists
  * Logical operators
  * Factors
  * Functions
  * Missingness
...
- You can convert character to numeric if it **ONLY** contains numbers
- You can convert character to logical **ONLY** if it is `"TRUE"` or `"FALSE"`
- If R does not know how to convert you get a missing value (`NA`)
...
- Notice that `R` is **recycling** `a` without letting us know!
...
- If we try to mix two types `R` will automatically *cast*:
...
## Data frames
To keep variables of different types in a 2D structure, the simplest answer is a **data frame**, a data structure from base-R
```{r}
...
## Lists in lists
- A construct more useful than it initially seems is a *list of lists*
- Here we have a list with two identical elements:
```{r}
g <- list(f, f)
```

- To call the vector from the second list within the list `g`, use either of the following lines of code:
```{r}
g[[2]][[1]]
g[[2]]$vector
```
...
## Factor
Can be useful for categorical variables:
```{r}
...
- Strings are sometimes converted to factors automatically (unless you use tibbles).
...
- The factor is a data structure which is used for fields that take only a predefined finite number of values. 
...
Here we can see there are 3 arguments:

  * `x`: An `R` object
  * `trim = 0`: Number that removes a fraction before the mean is calculated
  * `na.rm = FALSE`: Whether to remove `NA` from `x`.
...
mean(c(1, 2, NA, 4, 5))
```
Often there is an argument called `na.rm`:
```{r}
